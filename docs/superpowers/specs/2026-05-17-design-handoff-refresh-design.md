...
- Title row actions: `← / today / →` text nav buttons + `load date` calendar icon button
  ...
  CTA with count pill (navigates to `/list`; AddFromPlanModal already lives there)
...
| `add recipes to list` CTA | PlanPage | New CTA replacing old prop bar | Navigate to /list (AddFromPlanModal lives there) |
