...
- 2026-06-09 — Inventory item category corrections: edit-mode `ItemForm` now shows the existing canonical food category and submits category changes through `PUT /api/inventory/:id`. The server validates the category, proves the inventory row belongs to the household, then updates the associated curated `canonical_foods.category`; inventory updates also invalidate shopping-list caches because aisle grouping derives from the same category. Added focused Vitest coverage for form submission and server persistence plus Playwright coverage for editing an inventory item category; `pnpm test` and `pnpm test:e2e` passed.
